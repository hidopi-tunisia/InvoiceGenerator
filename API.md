...
```json
{
  ...
  "errors": [
    ...
    {
      "field": "fiscalIdentifier.value",
      "message": "Format MF invalide — attendu : XXX XXX XXX (9 chiffres)"
    },
...
| PATCH             | `/profile/logo`                      | ✅   | `{ logoBase64 }` (data URI) | `{ success, data: { logoUrl } }` — ⚠️ le backend déployé valide `logoBase64` en JSON, pas le multipart initialement documenté (constaté 2026-07-11) |
...
| `Content-Type: application/json`        | →         | Toutes les mutations, y compris `/profile/logo` (JSON `logoBase64` — cf. §157) ; multipart : `/profile/documents` |
...
```json
{
  ...
  "data": {
    ...
    "sender": "fWq8...uid",
    "recipient": {
      "_id": "6650f2a9c1e4a12b34567890",
      "companyName": "ACME SARL",
      "contactPerson": "Sami Ben Ali",
      "email": "sami@acme.tn"
    },
    "items": [
      { "label": "Développement site vitrine", "quantity": 1, "unitPrice": 1200 },
  ...
  "data": [
    {
      ...
      "isLate": true,
      "total": 890,
      "recipient": { "companyName": "ACME SARL" },
